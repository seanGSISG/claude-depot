...
- [Querying the API](#querying-the-api)
  - [Running a Script on an Agent Using the API](#running-a-script-on-an-agent-using-the-api)
- [API via CLI](#api-via-cli)
- [API Examples](#api-examples)
  - [Listing All Software on All Agents](#listing-all-software-on-all-agents)
  - [List Agents](#list-agents)
  - [List Tasks](#list-tasks)
  - [Get Custom Fields (PowerShell)](#get-custom-fields-powershell)
  - [Get Custom Fields (Python)](#get-custom-fields-python)
  - [Create Collector Tasks](#create-collector-tasks)
  - [Show Tasks](#show-tasks)
  - [Show Task/Collector Results](#show-taskcollector-results)
  - [List Custom Fields of Agent](#list-custom-fields-of-agent)
  - [Update Custom Fields](#update-custom-fields)
  - [List Service Information from Agent](#list-service-information-from-agent)
  - [Combining Scripts](#combining-scripts)
  - [Pulling Audit Logs](#pulling-audit-logs)
  - [Update Software Using API](#update-software-using-api)
...
However this makes it difficult to document the API as it has only been designed to be consumed by the vue frontend.
...
When creating the key you'll need to choose a user, which will reflect what permissions the key has based on the user's role. See the Permissions section in references/settings-and-admin.md for details on role-based access control.

Navigate to **Settings > Global Settings > API Keys** to generate a key. See the Global Settings section in references/settings-and-admin.md for more on this configuration area.

> **Warning:** Pay attention to your trailing `/` they matter.
...
### Python - Trigger Windows Update Scan
...
### PowerShell - Send Command to Agent
...
First get the script ID from the Script library by hovering your mouse over it in the script list and looking at the tooltip. See the Scripting section in references/scripting-and-variables.md for more on managing scripts.
...
The `script` field is the primary key of the script in PostgreSQL. The `timeout` is in seconds.

#### PowerShell - Run a Script on an Agent
...
A community CLI tool is available at: https://gitlab.com/NiceGuyIT/trmm-cli
...
See the Software section in references/settings-and-admin.md for more on software management.
...
See the Automated Tasks section in references/checks-tasks-policies.md for task configuration details.
...
### Get Custom Fields (PowerShell)

See the Custom Fields section in references/scripting-and-variables.md for custom field configuration.
...
See the Automated Tasks section in references/checks-tasks-policies.md for collector task configuration.
...
This will update the custom fields for an agent. The field value comes from the custom fields listed above. Other fields that exist will not be updated. If you want to update only 1 custom field, use only 1 entry in the array.
...
This will iterate over all agents, filter out non-Windows and offline agents, run the PowerShell above to get the first 5 services that are running, and compile the information in a Servers array.
...
See the Software section in references/settings-and-admin.md for details on software management.
