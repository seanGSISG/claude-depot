...
on_post_tool_use() {
  ...
  sqlite3 "$GLOBAL_DB" \
    "INSERT INTO tool_log (tool_name, result) VALUES ('$(echo "$tool_name" | tr "'" "''")','success');" 2>/dev/null || true
}
...
on_post_tool_use_failure() {
  ...
  # Log failure to global
  sqlite3 "$GLOBAL_DB" \
    "INSERT INTO tool_log (tool_name, result) VALUES ('$(echo "$tool_name" | tr "'" "''")','failure');" 2>/dev/null || true
}
...
on_session_end() {
  ...
  # Status to stderr only — don't pollute Claude's context
  echo "Session reflected — tools: $total, failures: $failures" >&2
  if [[ -n "$fail_tools" ]]; then
    echo "  Stored failure patterns for: $(echo "$fail_tools" | tr '\n' ', ')" >&2
  fi
}
